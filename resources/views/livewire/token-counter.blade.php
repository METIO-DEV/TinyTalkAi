    ...
    @if($selectedModel)
        <div class="flex flex-col space-y-2">
            <div class="flex justify-center gap-8 items-center text-xs text-custom-black dark:text-custom-white">
                ...
                <div class="w-[60%] bg-custom-mid rounded-full h-4 dark:bg-gray-700 flex items-center">
                    ...
                    <span class="text-xs relative 
                        @if($this->getTokenPercentageProperty() == 0)
                            left-0
                        @elseif($this->getTokenPercentageProperty() < 10)
                            right-6
                        @else
                            right-8
                        @endif
                        text-custom-black dark:text-custom-white">{{ $this->getTokenPercentageProperty() }}%</span>
                </div>
                <button 
